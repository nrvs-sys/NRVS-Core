...
public static class DebugExtension
{
	...
	/// <summary>
	/// 	- Debugs a cone with a specified length.
	/// </summary>
	/// <param name='position'>
	/// 	- The position for the tip of the cone.
	/// </param>
	...
	/// 	- Whether or not the cone should be faded when behind other objects.
	/// </param>
	public static void DebugCone(Vector3 position, Vector3 direction, Color color, float angle = 45, float length = 1, float duration = 0, bool depthTest = true)
	{
		Vector3 _forward = direction.normalized * length;
		Vector3 _up = Vector3.Slerp(_forward, -_forward, 0.5f);
		Vector3 _right = Vector3.Cross(_forward, _up).normalized * length;
		...
		float dist;
		var farPlane = new Plane(-direction.normalized, position + _forward);
		var distRay = new Ray(position, slerpedVector);
	...
	/// </param>
	public static void DebugArrow(Vector3 position, Vector3 direction, float length, Color color, float duration = 0, bool depthTest = true)
	{
		Debug.DrawLine(position, position + direction * length, color, duration, depthTest);
		DebugExtension.DebugCone(position + direction * length, -direction*0.333f, color, 15, duration, depthTest);
	}
	...
	/// 	- Whether or not the arrow should be faded when behind other objects. 
	/// </param>
	public static void DebugArrow(Vector3 position, Vector3 direction, float length, float duration = 0, bool depthTest = true)
	{
		DebugArrow(position, direction, length, Color.white, duration, depthTest);
	}
